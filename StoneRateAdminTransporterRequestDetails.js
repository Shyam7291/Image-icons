...
function Toast({ toast }) {
  return (
    <div
      className={"trd-toast" + (toast ? " show t-" + toast.tone : "")}
...
function ResponseTimer({ request, now, compact = false }) {
  ...
  if (level === "closed") {
    return (
      <div className={"trd-timer final lvl-closed" + (compact ? " compact" : "")}>
        <span className="trd-timer-emblem"><Icon name="lock" size={20} /></span>
        <div>
          <span className="trd-kicker">Response window</span>
          <strong>Response window closed</strong>
          <small>Expired {formatDateTime(request.expiresAt)}</small>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={"trd-timer live lvl-" + level + (compact ? " compact" : "")}>
      ...
      <small className="trd-timer-foot">Closes {formatDateTime(request.expiresAt)}</small>
    </div>
  );
}
  ...
  return (
    <header className="trd-header">
      <div className="trd-width trd-header-inner">
        <div className="trd-top">
          ...
          <div className="trd-brand">
            <strong aria-hidden="true">
              <Icon name="cube" size={17} strokeWidth={1.9} />
              <i className="trd-brand-ping" />
            </strong>
            <span>
              <b>StoneRate Admin</b>
              <small>Transporter Desk</small>
            </span>
          </div>
          <button
        ...
        <h1 className="trd-title">
          Transporter <span>Request Details</span>
        </h1>
        <p className="trd-sub">Review the material request, contact the parties, and record the Admin decision</p>

        <div className="trd-idline">
          {request ? (
            <>
              <span className="trd-idchip">
                <Icon name="hash" size={13} />
                <b className={request.id ? "" : "trd-missing"}>{request.id || "Request ID unavailable"}</b>
              </span>
              <StatusBadge status={request.status} label={request.statusLabel} />
            </>
          ) : (
            <>
              <SkeletonBlock className="sk-idchip" />
...
function AcceptRequestModal({ open, onClose, request, now, busy, error, onConfirm }) {
  ...
  const remainingLabel =
    remaining === null ? "Response deadline not provided" : remaining <= 0 ? "Response window closed" : durationLabel(remaining);
  return (
    <Modal
      open={open}
      onClose={onClose}
...
function RejectRequestModal({ open, onClose, request, busy, error, onSubmit, windowOpen }) {
  ...
  const footer =
    step === "select" ? (
      <>
        ...
        <button type="button" className="trd-btn trd-btn-danger" onClick={goConfirm} disabled={busy || !windowOpen}>
          <span>Continue</span>
      ...
      <>
        <button type="button" className="trd-btn trd-btn-ghost" onClick={() => setStep("select")} disabled={busy}>
          Keep Request Open
        </button>
        <button type="button" className="trd-btn trd-btn-danger solid" onClick={submit} disabled={busy || !windowOpen} data-autofocus>
  ...
  return (
    ...
    >
      {step === "select" ? (
        <>
          <p className="trd-modal-lead">Select the reason that best explains why this request cannot be accepted.</p>
            ...
            {REJECTION_REASONS.map(item => {
              ...
              return (
                <label key={item.value} className={"trd-reason" + (active ? " active" : "") + (busy ? " disabled" : "")}>
                  ...
                  />
                  <span className="trd-reason-icon" aria-hidden="true"><Icon name={item.icon} size={16} /></span>
          ...
          {reason ? (
            <div className={"trd-notes" + (touched && noteMissing ? " invalid" : "")}>
              <label htmlFor="trd-reject-notes">
                Rejection notes{reason === "other" ? <em> Required</em> : <small> Optional Admin note</small>}
              </label>
              <textarea
                id="trd-reject-notes"
                ref={noteRef}
              ...
              <div className="trd-notes-meta" id="trd-reject-notes-meta">
                <span>
                  {touched && noteMissing ? (
                    <b className="trd-field-error inline"><Icon name="alert" size={12} />Rejection notes are required for Other.</b>
                  ) : reason === "other" ? (
                    "Describe the reason clearly for the request history."
                  ) : (
        ...
        <div className="trd-final-warn">
          <span className="trd-final-icon" aria-hidden="true"><Icon name="alert" size={24} strokeWidth={1.9} /></span>
          <h3>Confirm rejection</h3>
          <p>Rejecting this request will stop the current material-request workflow for the Transporter.</p>
          <dl className="trd-rows boxed">
            <InfoRow label="Reason" value={selected ? selected.label : null} strong />
            {trimmed ? <InfoRow label="Note" value={trimmed} /> : null}
          </dl>
        </div>
      )}
      {!windowOpen ? (
        <div className="trd-inline-error" role="alert">
          ...
          <span>Response window closed. This request can no longer be rejected from here.</span>
        </div>
      ) : null}
      {error ? (
        <div className="trd-inline-error" role="alert">
...
function DecisionPanel({ request, now, busy, onAccept, onReject }) {
  ...
  const actionable = status === "new" && !windowClosed;
  ...
  else if (windowClosed) message = { tone: "muted", icon: "lock", text: "Response window closed" };
  ...
  return (
    ...
    >
      <div className="trd-decision-copy">
        ...
        <p>Review the Transporter, material reference, and Seller availability before recording the final decision.</p>
        {actionable && noDeadline ? (
          <small className="trd-hint"><Icon name="info" size={12} />Response deadline not provided — the backend will validate the window.</small>
        ) : null}
      </div>
      {message ? (
...
function BottomNav({ items }) {
  return (
    <nav className="trd-bottomnav" aria-label="Admin primary navigation">
      {items.map(item => (
        <button
          key={item.key}
          type="button"
          className={"trd-navitem" + (item.active ? " active" : "")}
          aria-current={item.active ? "page" : undefined}
          onClick={() => safeCall(item.action)}
        >
          <span className="trd-nav-icon"><Icon name={item.icon} size={17} /></span>
          <span className="trd-nav-label">{item.label}</span>
        </button>
      ))}
    </nav>
  );
...
export default function StoneRateAdminTransporterRequestDetails({
  ...
  const windowOpen = Boolean(
    request && request.status === "new" && (request.expiresAt === null || request.expiresAt - now > 0)
  );
  ...
  const confirmAccept = async () => {
    ...
    if (!windowOpen) {
      setActionError("Response window closed. This request can no longer be accepted.");
      return;
    }
  ...
  const submitReject = async ({ reasonCode, reasonLabel, note }) => {
    if (!request || submitLock.current || saving) return;
    if (!windowOpen) {
      setActionError("Response window closed. This request can no longer be rejected.");
      return;
    }
    submitLock.current = true;
  ...
  const navItems = [
    { key: "samples", label: "Samples", icon: "samples", action: onSamples },
    { key: "transport", label: "Transport", icon: "truck", action: onTransporterRequests, active: true },
    { key: "home", label: "Home", icon: "home", action: onHome },
    { key: "rates", label: "Rates", icon: "doc", action: onRateRequests },
    { key: "orders", label: "Orders", icon: "clipboard", action: onConfirmedOrders }
  ];

...
/* ============================================================
   IMAGE ICONS (3D artwork hosted on GitHub via jsDelivr)
   ============================================================ */

const ICON_CDN = "https://cdn.jsdelivr.net/gh/shyam7291/Image-icons@main";

const IMAGE_ICONS = {
  // Quick actions
  upload: `${ICON_CDN}/uploadsample.png`,
  rates: `${ICON_CDN}/02-update-rates-coins.png`,
  orders: `${ICON_CDN}/03-view-orders-clipboard.png`,
  availability: `${ICON_CDN}/04-set-availability-calendar.png`,
  // Performance metrics
  salesRecords: `${ICON_CDN}/05-sales-records-book.png`,
  tonsSold: `${ICON_CDN}/06-tons-sold-truck.png`,
  activeOrders: `${ICON_CDN}/07-active-orders-timer.png`,
  dispatched: `${ICON_CDN}/08-dispatched-completed-box.png`,
};

/**
 * Resolve the artwork for a performance metric.
 * The metric payload comes from the API, so we match on id, label and icon
 * name to stay resilient to whichever field the backend provides.
 */
function performanceImage(metric = {}) {
  const key = `${metric.id || ""} ${metric.label || ""} ${metric.icon || ""}`.toLowerCase();
  if (/dispatch|complet|deliver|check/.test(key)) return IMAGE_ICONS.dispatched;
  if (/active|pending|progress|activity|timer/.test(key)) return IMAGE_ICONS.activeOrders;
  if (/ton|weight|sold|truck|quantity/.test(key)) return IMAGE_ICONS.tonsSold;
  if (/sale|record|revenue|book|rates/.test(key)) return IMAGE_ICONS.salesRecords;
  return null;
}
...
function ImageIcon({ src, alt = "", className = "" }) {
  return (
    <img
      className={`img-icon ${className}`.trim()}
      src={src}
      alt={alt}
      draggable="false"
      loading="lazy"
...
function QuickActions({ onNavigate }) {
  const actions = [
    { id: "upload", label: "Upload Sample", icon: "upload", tone: "orange", image: IMAGE_ICONS.upload },
    { id: "rates", label: "Update Rates", icon: "rates", tone: "cyan", image: IMAGE_ICONS.rates },
    { id: "orders", label: "View Orders", icon: "orders", tone: "violet", image: IMAGE_ICONS.orders },
    { id: "availability", label: "Set Availability", icon: "availability", tone: "green", image: IMAGE_ICONS.availability },
  ];

  return (
    <section className="quick-actions quick-actions-imaged" aria-label="Seller quick actions">
      {actions.map((action, index) => (
        ...
        >
          <span className="qa-art">
            {action.image ? (
              <ImageIcon src={action.image} alt="" className="qa-img" />
            ) : (
...
function PerformanceCard({ metric, index }) {
  ...
  const artwork = metric.image || performanceImage(metric);

  return (
    <Reveal delay={index * 70}>
      <article className={`performance-card tilt ${artwork ? "has-art" : ""}`} {...tilt}>
        <span className="feature-glare" aria-hidden="true" />
        <span className="card-edge" aria-hidden="true" />
        <span className="metric-wave" aria-hidden="true" />
        {artwork ? (
          <div className="metric-art layer-3">
            <ImageIcon src={artwork} alt="" className="metric-img" />
          </div>
        ) : (
...
const CSS_IMAGE_ICONS = `
/* ============================================================
   IMAGE ICONS
   3D artwork replaces the stroke icons in Quick Actions and
   This Month Performance, matching the supplied reference.
   ============================================================ */
.img-icon{display:block;max-width:100%;max-height:100%;object-fit:contain;object-position:center;
  pointer-events:none;user-select:none;-webkit-user-drag:none}

/* ---------- quick actions: tall cards, artwork fills the top ---------- */
.quick-actions-imaged{gap:9px}
.quick-actions-imaged button{
  gap:8px;
  padding:12px 6px 13px;
  border-radius:20px;
  border-width:1.5px;
  background:linear-gradient(165deg,rgba(255,255,255,.99),rgba(246,243,255,.97));
}
.quick-actions-imaged button::after{opacity:0}
/* The badge chip becomes a transparent stage so the artwork stands alone. */
.quick-actions-imaged button>.qa-art{
  width:100%;
  height:64px;
  display:grid;
  place-items:center;
...
.quick-actions-imaged .qa-img{
  width:64px;
  height:64px;
  filter:drop-shadow(0 8px 12px rgba(58,42,110,.20));
}
.quick-actions-imaged button:hover>.qa-art{transform:translateY(-2px) scale(1.04)}
.quick-actions-imaged button:hover span{transform:translateY(-2px) scale(1.04)}
.quick-actions-imaged button>b{font-size:8.5px;line-height:1.25}

/* ---------- performance cards: large artwork above the number ---------- */
.performance-card.has-art{padding:14px 13px 15px}
.metric-art{
  width:100%;
  height:96px;
  display:flex;
...
.metric-img{
  ...
  filter:drop-shadow(0 10px 14px rgba(58,42,110,.18));
  transition:transform .4s cubic-bezier(.22,.61,.36,1);
}
.performance-card.has-art:hover .metric-img{transform:translateY(-2px) scale(1.03)}
.performance-card.has-art strong{margin-top:4px;font-size:24px}

/* ---------- narrow phones ---------- */
@media(max-width:370px){
  .quick-actions-imaged button{padding:10px 4px 11px;gap:6px}
  .quick-actions-imaged button>.qa-art{height:54px}
  .quick-actions-imaged .qa-img{width:54px;height:54px}
  .quick-actions-imaged button>b{font-size:7.5px}
  .metric-art{height:82px}
  .metric-img{width:90px;height:82px}
  .performance-card.has-art strong{font-size:21px}
}

/* Slightly larger artwork when the phone frame is displayed on desktop. */
@media(min-width:700px){
  .quick-actions-imaged button>.qa-art{height:70px}
  .quick-actions-imaged .qa-img{width:70px;height:70px}
}
`;

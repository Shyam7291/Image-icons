...
/* ---------- Transporter view config ---------- */
const VIEWS = { BUYER: "BUYER", TRANSPORTER: "TRANSPORTER" };
const TP_FILTERS = ["ALL", "NEW", "LOADING", "DELIVERED"];
const TP_LABELS = { ALL: "All", NEW: "New", LOADING: "Loading", DELIVERED: "Delivered" };
const TP_STATUS_CLASS = { NEW: "new", LOADING: "loading", DELIVERED: "delivered" };
const TP_STEPS = ["NEW", "LOADING", "DELIVERED"];
...
function Icon({ name, size = 18, strokeWidth = 1.8 }) {
  ...
  const paths = {
    ...
    store: <><path d="M4 9 5.5 4h13L20 9"/><path d="M4 9h16v1.5a2.7 2.7 0 0 1-5.3.7 2.7 2.7 0 0 1-5.4 0 2.7 2.7 0 0 1-5.3-.7z"/><path d="M5.5 13v7h13v-7M10 20v-4h4v4"/></>,
...
function normalizeTripStatus(status) {
  ...
  if (value === "NEW" || value === "CONFIRMED" || value === "ASSIGNED") return "NEW";
  return "LOADING"; // LOADING, IN_TRANSIT, DISPATCHED -> active loading/movement stage
}
function idSuffix(id) {
  const match = String(id || "").match(/(\d{6}-\d{3,})$/);
  return match ? match[1] : null;
}
function toTransporterTrip(order, index) {
  const suffix = idSuffix(order.tripId) || idSuffix(order.deliveryId) || idSuffix(order.requestId) || String(index + 1).padStart(3, "0");
  const materials = Array.isArray(order.materials) && order.materials.length
    ? order.materials.map(item => ({ name: item.materialName || item.name || "Material", tons: Number(item.totalTons ?? item.quantity ?? 0) || 0 }))
    : [{ name: order.materialName || "Material", tons: Number(order.quantity ?? order.totalTons ?? 0) || 0 }];
  return {
    tripId: order.tripId || order.transportRequestId || `TRTR-${suffix}`,
    deliveryId: order.transportDeliveryId || (/^TD-/.test(order.deliveryId || "") ? order.deliveryId : `TD-${suffix}`),
    status: normalizeTripStatus(order.transportStatus || order.status),
    materials,
    transporter: order.transporter || null,
    seller: order.seller || null,
    location: order.location || order.pickupLocation || order.deliveryArea || null,
    confirmedAt: order.transportConfirmedAt || order.confirmedAt || null,
    updatedAt: order.updatedAt || order.confirmedAt || null,
    vehicleCount: Number(order.vehicleCount || 0),
    source: order
  };
}
function tripTons(trip) { return trip.materials.reduce((sum, item) => sum + item.tons, 0); }
function formatDateTime(iso) {
  if (!iso) return "--";
  const date = new Date(iso);
  if (Number.isNaN(date.getTime())) return "--";
  return new Intl.DateTimeFormat("en-IN", { day: "2-digit", month: "short", hour: "numeric", minute: "2-digit", hour12: true }).format(date);
}

function TransporterCard({ trip, now, onOpen, index }) {
  const statusClass = TP_STATUS_CLASS[trip.status];
  const stepIndex = TP_STEPS.indexOf(trip.status);
  const shown = trip.materials.slice(0, 2);
  const hidden = trip.materials.length - shown.length;
  const handleKeyDown = event => {
    if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onOpen(trip); }
  };
  return <article className={`ao-card tp-card status-${statusClass}`} style={{ animationDelay: `${index * 55}ms` }} role="button" tabIndex="0" onClick={() => onOpen(trip)} onKeyDown={handleKeyDown} aria-label={`Open transport order ${trip.tripId}`}>
    <span className="ao-card-rail"/><span className="ao-card-mesh"/>
    <header className="ao-card-head">
      <div className="tp-ids"><strong>{trip.tripId}</strong><span><Icon name="link" size={10}/>{trip.deliveryId}</span></div>
      <span className={`ao-badge ${statusClass}`}><i/>{TP_LABELS[trip.status]}</span>
    </header>
    <div className="tp-materials">
      {shown.map((material, materialIndex) => <div className="tp-mat" key={`${material.name}-${materialIndex}`}>
        <span className="tp-mat-icon"><Icon name="cube" size={13}/></span>
        <span className="tp-mat-name">{material.name}</span>
        <span className="tp-qty">{material.tons}<em>t</em></span>
      </div>)}
      {hidden > 0 ? <div className="tp-more">+{hidden} more material{hidden === 1 ? "" : "s"} · {tripTons(trip)} t total</div> : null}
    </div>
    <div className="tp-grid">
      <div className={`tp-info${!trip.transporter ? " missing" : ""}`}><span className="tp-ico"><Icon name="truck" size={13}/></span><div><small>Transporter</small><strong title={trip.transporter || ""}>{trip.transporter || "Not assigned"}</strong></div></div>
      <div className={`tp-info seller${!trip.seller ? " missing" : ""}`}><span className="tp-ico"><Icon name="store" size={13}/></span><div><small>Seller</small><strong title={trip.seller || ""}>{trip.seller || "Not assigned"}</strong></div></div>
      <div className="tp-info loc wide"><span className="tp-ico"><Icon name="pin" size={13}/></span><div><small>Location</small><strong title={trip.location || ""}>{trip.location || "Not recorded"}</strong></div>{trip.vehicleCount ? <b className="tp-veh"><Icon name="truck" size={11}/>{trip.vehicleCount}</b> : null}</div>
    </div>
    <div className="tp-track" aria-label={`Progress: ${TP_LABELS[trip.status]}`}>
      {TP_STEPS.map((step, stepNo) => <span key={step} className={stepNo <= stepIndex ? "done" : ""}><i/>{TP_LABELS[step]}</span>)}
    </div>
    <footer className="ao-card-foot">
      <div className="tp-when"><span><Icon name="clock" size={14}/></span><div><small>Confirmed</small><strong>{formatDateTime(trip.confirmedAt)}</strong><em>{relativeTime(trip.confirmedAt, now)}</em></div></div>
      <span className="ao-open">View Order <Icon name="arrow" size={15}/></span>
    </footer>
  </article>;
}
...
export default function AdminActiveOrders({ onBack, onOpenOrder, onHome, onOrders, onSamples, onTransporterBidding, onRateRequests, onConfirmedOrders }) {
  ...
  const [view, setView] = useState(VIEWS.BUYER);
  ...
  /* ---------- Transporter view data ---------- */
  const trips = useMemo(() => orders.map((order, index) => toTransporterTrip(order, index)), [orders]);
  const dateTrips = useMemo(() => trips.filter(trip => matchesDateFilter(trip, dateFilter, now)), [trips, dateFilter, now]);
  const tpCounts = useMemo(() => {
    const result = { ALL: dateTrips.length, NEW: 0, LOADING: 0, DELIVERED: 0 };
    dateTrips.forEach(trip => { result[trip.status] += 1; });
    return result;
  }, [dateTrips]);
  const tpStats = useMemo(() => {
    const awaiting = trips.filter(trip => trip.status === "NEW");
    const loadingTrips = trips.filter(trip => trip.status === "LOADING");
    const unassigned = awaiting.filter(trip => !trip.transporter).length;
    const vehicles = loadingTrips.reduce((sum, trip) => sum + trip.vehicleCount, 0);
    const tonsMoving = trips.filter(trip => trip.status !== "DELIVERED").reduce((sum, trip) => sum + tripTons(trip), 0);
    const partners = new Set(trips.filter(trip => trip.transporter && trip.status !== "DELIVERED").map(trip => trip.transporter)).size;
    const delivered = trips.filter(trip => trip.status === "DELIVERED").length;
    const onTimeRate = trips.length ? Math.round((delivered / trips.length) * 100) : 0;
    return { awaiting: awaiting.length, unassigned, loading: loadingTrips.length, vehicles, tonsMoving, partners, delivered, onTimeRate };
  }, [trips]);
  const visibleTrips = useMemo(() => {
    const normalized = query.trim().toLowerCase();
    const matched = dateTrips.filter(trip => {
      const statusMatch = tpFilter === "ALL" || trip.status === tpFilter;
      const text = [trip.tripId, trip.deliveryId, trip.transporter || "", trip.seller || "", trip.location || "", ...trip.materials.map(item => item.name)].join(" ").toLowerCase();
      return statusMatch && (!normalized || text.includes(normalized));
    });
    const statusRank = { NEW: 0, LOADING: 1, DELIVERED: 2 };
    return [...matched].sort((a, b) => {
      if (sort === "newest") return new Date(b.confirmedAt) - new Date(a.confirmedAt);
      if (sort === "oldest") return new Date(a.confirmedAt) - new Date(b.confirmedAt);
      if (sort === "volume") return tripTons(b) - tripTons(a);
      if (sort === "status") return statusRank[a.status] - statusRank[b.status];
      const aMissing = !a.seller || !a.transporter ? 0 : 1;
      const bMissing = !b.seller || !b.transporter ? 0 : 1;
      return aMissing - bMissing || statusRank[a.status] - statusRank[b.status] || new Date(a.updatedAt) - new Date(b.updatedAt);
    });
  }, [dateTrips, query, tpFilter, sort]);
  const isTransporterView = view === VIEWS.TRANSPORTER;
  const activeFilterLabel = isTransporterView ? TP_LABELS[tpFilter] : FILTER_LABELS[filter];
  const resultCount = isTransporterView ? visibleTrips.length : visibleOrders.length;
  const resultNoun = isTransporterView ? "trip" : "order";
  ...
  const handleOpenTrip = trip => onOpenOrder ? onOpenOrder(trip.source, { view: "transporter", trip }) : showToast(`Opening ${trip.tripId}`);
  const switchView = next => { if (next !== view) { setView(next); setQuery(""); } };
  ...
  return <div className="ao-root">
    ...
    <header className="ao-header"><div className="ao-header-inner">
      ...
      <div className="ao-hero"><h1>Active <span>Orders</span></h1><p>{isTransporterView ? "Track transporter trips from pickup to delivery." : "Manage assignments, loading, transit, and delivery."}</p><small><i/> Updated {relativeTime(new Date(refreshedAt).toISOString(), now)}</small></div>
      <div className={`ao-view${isTransporterView ? " tp" : ""}`} role="tablist" aria-label="Order view">
        <span className="ao-view-pill" aria-hidden="true"/>
        <button type="button" role="tab" aria-selected={!isTransporterView} className={!isTransporterView ? "active" : ""} onClick={() => switchView(VIEWS.BUYER)}><Icon name="user" size={15}/><span>By Buyer</span><b>{orders.length}</b></button>
        <button type="button" role="tab" aria-selected={isTransporterView} className={isTransporterView ? "active" : ""} onClick={() => switchView(VIEWS.TRANSPORTER)}><Icon name="truck" size={15}/><span>By Transporter</span><b>{trips.length}</b></button>
      </div>
      {isTransporterView ? <section className="ao-metrics" aria-label="Transporter summary" key="tp-metrics">
        <Metric icon="inbox" label="Awaiting Pickup" value={tpStats.awaiting} hint={`${tpStats.unassigned} without transporter`} tone="orange" bars={[36,52,48,70,82]}/>
        <Metric icon="truck" label="Loading Now" value={tpStats.loading} hint={`${tpStats.vehicles} vehicles engaged`} tone="red" bars={[44,62,55,74,68]}/>
        <Metric icon="volume" label="Tonnage in Motion" value={money(tpStats.tonsMoving)} unit="t" hint={`${tpStats.partners} active transporters`} tone="blue" bars={[40,58,66,72,90]}/>
        <Metric icon="check" label="Trips Delivered" value={tpStats.delivered} hint={`${tpStats.onTimeRate}% of all trips completed`} tone="green" bars={[52,66,74,80,94]}/>
      </section> : <section className="ao-metrics" aria-label="Active order summary" key="buyer-metrics">
    ...
    <main className="ao-shell">
      <section className="ao-toolbar">
        <div className="ao-toolbar-top">
          <label className="ao-search"><Icon name="search" size={16}/><input ref={searchRef} value={query} onChange={event => setQuery(event.target.value)} placeholder={isTransporterView ? "Search trip, transporter, seller, material or location" : "Search delivery, request, material or area"} aria-label={isTransporterView ? "Search transporter trips" : "Search active orders"}/>{query ? <button type="button" onClick={() => { setQuery(""); searchRef.current?.focus(); }} aria-label="Clear search"><Icon name="close" size={13}/></button> : null}</label>
        ...
        <div className="ao-filters" role="tablist" aria-label="Delivery status">
          {isTransporterView
            ? TP_FILTERS.map(item => <button key={item} type="button" role="tab" aria-selected={tpFilter === item} className={`ao-filter s-${TP_STATUS_CLASS[item] || "all"}${tpFilter === item ? " active" : ""}`} onClick={() => setTpFilter(item)}><i/>{TP_LABELS[item]}<span>{tpCounts[item]}</span></button>)
      ...
      <div className="ao-result"><strong><Icon name={isTransporterView ? "truck" : "activity"} size={13}/>{loading ? `Loading ${resultNoun}s` : `${resultCount} ${resultNoun}${resultCount === 1 ? "" : "s"}`}</strong><span>{isTransporterView ? "Transporter view · " : ""}{activeFilterLabel === "All" ? "All statuses" : activeFilterLabel} · {sort === "attention" ? "Attention first" : "Sorted"}</span></div>
      {error ? <section className="ao-state error"><span><Icon name="alert" size={22}/></span><h2>Could not load active orders</h2><p>{error}</p><button type="button" onClick={retry}>Retry</button></section>
        : loading ? <section className="ao-list"><SkeletonCard/><SkeletonCard/><SkeletonCard/><SkeletonCard/></section>
        : isTransporterView
          ? (visibleTrips.length === 0
            ? <section className="ao-state"><span><Icon name="truck" size={22}/></span><h2>{query ? `No trips found for “${query}”.` : "No transporter trips match this filter."}</h2><p>Try another status, date range, or search term.</p>{query ? <button type="button" onClick={() => setQuery("")}>Clear search</button> : null}</section>
            : <section className="ao-list tp-list" aria-label="Transporter trips">{visibleTrips.map((trip, index) => <TransporterCard key={`${trip.tripId}-${trip.deliveryId}`} trip={trip} now={now} onOpen={handleOpenTrip} index={index}/>)}</section>)
          : visibleOrders.length === 0 ? <section className="ao-state"><span><Icon name="inbox" size={22}/></span><h2>{query ? `No orders found for “${query}”.` : "No active orders match this filter."}</h2><p>Try another status, date range, or search term.</p>{query ? <button type="button" onClick={() => setQuery("")}>Clear search</button> : null}</section> : <section className="ao-list" aria-label="Active orders">{visibleOrders.map((order, index) => <OrderCard key={order.deliveryId} order={order} now={now} onOpen={handleOpen} index={index}/>)}</section>}
    </main>
...
.ao-view{position:relative;display:grid;grid-template-columns:1fr 1fr;gap:0;margin-top:14px;padding:4px;border:1px solid var(--line2);border-radius:14px;background:var(--glass);backdrop-filter:blur(14px);box-shadow:0 6px 18px rgba(20,26,36,.06),inset 0 1px 0 rgba(255,255,255,.9)}.ao-view-pill{position:absolute;z-index:0;left:4px;top:4px;bottom:4px;width:calc(50% - 4px);border-radius:10px;background:linear-gradient(135deg,var(--orange),var(--orange2));box-shadow:0 6px 16px rgba(249,115,22,.32),inset 0 1px 0 rgba(255,255,255,.25);transition:transform .34s cubic-bezier(.25,1.12,.45,1)}.ao-view.tp .ao-view-pill{transform:translateX(100%)}.ao-view button{position:relative;z-index:1;height:38px;display:flex;align-items:center;justify-content:center;gap:7px;border:0;border-radius:10px;background:transparent;color:var(--muted);font-size:12.5px;font-weight:750;cursor:pointer;transition:color .22s}.ao-view button:hover{color:var(--ink)}.ao-view button.active{color:#fff}.ao-view button b{min-width:20px;padding:1px 6px;border-radius:6px;background:#eef1f6;color:var(--muted);font-size:10px;font-weight:800;transition:.22s}.ao-view button.active b{background:rgba(255,255,255,.22);color:#fff}
.tp-card{padding:12px 12px 11px 15px}.tp-card:focus-visible{outline:3px solid rgba(249,115,22,.35);outline-offset:2px}.tp-ids{display:flex;flex-direction:column;gap:4px;min-width:0}.tp-ids strong{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:13px;letter-spacing:-.01em;color:var(--ink)}.tp-ids span{display:inline-flex;align-items:center;gap:4px;align-self:flex-start;padding:2px 7px;border:1px solid var(--line2);border-radius:6px;background:#f7f9fc;color:var(--ink2);font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;font-weight:650}.tp-ids span svg{color:var(--orange)}
.tp-materials{position:relative;display:grid;gap:5px;margin-top:10px}.tp-mat{display:flex;align-items:center;gap:8px;padding:6px 7px 6px 6px;border:1px solid var(--line);border-radius:10px;background:linear-gradient(90deg,#fff,#fbfcfe)}.tp-mat-icon{width:24px;height:24px;display:grid;place-items:center;flex:0 0 auto;border-radius:7px;color:var(--orange2);background:var(--orangeSoft)}.tp-mat-name{flex:1;min-width:0;font-size:12.5px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tp-qty{flex:0 0 auto;padding:3px 8px;border-radius:7px;background:#141a24;color:#fff;font-size:11.5px;font-weight:800;font-variant-numeric:tabular-nums}.tp-qty em{font-style:normal;margin-left:2px;font-size:9.5px;opacity:.7}.tp-more{padding:0 4px;font-size:10px;font-weight:700;color:var(--faint)}
.tp-grid{position:relative;display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:9px}.tp-info{display:flex;align-items:center;gap:7px;min-width:0;padding:7px 8px;border-radius:10px;background:rgba(37,99,235,.05);border:1px solid rgba(37,99,235,.10)}.tp-info.seller{background:rgba(13,148,136,.05);border-color:rgba(13,148,136,.12)}.tp-info.loc{background:rgba(249,115,22,.05);border-color:rgba(249,115,22,.14)}.tp-info.wide{grid-column:1/-1}.tp-ico{width:24px;height:24px;display:grid;place-items:center;flex:0 0 auto;border-radius:7px;background:#fff;color:var(--blue);box-shadow:0 1px 3px rgba(20,26,36,.08)}.tp-info.seller .tp-ico{color:var(--teal)}.tp-info.loc .tp-ico{color:var(--orange2)}.tp-info>div{display:flex;flex-direction:column;min-width:0;flex:1}.tp-info small{font-size:8.5px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--faint)}.tp-info strong{font-size:11.5px;font-weight:700;color:var(--ink);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.tp-info.missing{background:repeating-linear-gradient(135deg,rgba(214,69,69,.05) 0 6px,rgba(214,69,69,.02) 6px 12px);border:1px dashed rgba(214,69,69,.35)}.tp-info.missing .tp-ico{color:var(--red)}.tp-info.missing strong{color:var(--red)}.tp-veh{display:inline-flex;align-items:center;gap:3px;flex:0 0 auto;padding:2px 7px;border-radius:999px;background:#fff;color:var(--ink2);font-size:10px;font-weight:800;box-shadow:0 1px 3px rgba(20,26,36,.08)}
.tp-track{position:relative;display:grid;grid-template-columns:repeat(3,1fr);gap:4px;margin-top:10px}.tp-track span{display:flex;flex-direction:column;gap:4px;font-size:8.5px;font-weight:800;letter-spacing:.06em;text-transform:uppercase;color:var(--faint)}.tp-track i{height:4px;border-radius:999px;background:#e8ecf2}.tp-track span.done{color:var(--ink2)}.tp-card.status-new .tp-track span.done i{background:linear-gradient(90deg,#60a5fa,var(--blue))}.tp-card.status-loading .tp-track span.done i{background:linear-gradient(90deg,#fbbf24,var(--amber))}.tp-card.status-delivered .tp-track span.done i{background:linear-gradient(90deg,#34d399,var(--green))}
.tp-card .ao-card-foot{padding-top:10px;border-top:1px dashed var(--line2)}.tp-when{display:flex;align-items:center;gap:8px;min-width:0}.tp-when>span{width:30px;height:30px;display:grid;place-items:center;flex:0 0 auto;border-radius:9px;background:#f1f4f8;color:var(--muted)}.tp-when>div{display:flex;flex-direction:column;min-width:0}.tp-when small{font-size:8.5px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:var(--faint)}.tp-when strong{font-size:11.5px;color:var(--ink);white-space:nowrap}.tp-when em{font-style:normal;font-size:9.5px;color:var(--faint)}.tp-card:hover .ao-open svg{transform:translateX(5px)}.ao-open svg{transition:transform .2s}

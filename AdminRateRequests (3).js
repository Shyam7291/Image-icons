...
/* Maps the transporter response level onto the buyer ring levels. */
function trRingLevel(level) {
  if (level === "critical") return "critical";
  if (level === "urgent") return "warning";
  return "steady";
}

function trCountdown(ms) {
  const parts = splitDuration(ms);
  return parts.hh + ":" + parts.mm + ":" + parts.ss;
}

/* Compact Transporter card — mirrors the "By Buyer" RequestCard layout. */
function TransporterRequestCard({ request, now, index, onView }) {
  const cls = TR_STATUS_CLASS[request.status] || "expired";
  const hasQuantity = request.quantity !== null;
  const requestedIso = request.requestedAt !== null ? new Date(request.requestedAt).toISOString() : null;
  const requestedFull = formatIndianDateTime(request.requestedAt);

  const isLive = request.status === "new" && request.deadline !== null && request.deadline - now > 0;
  const remaining = request.deadline !== null ? request.deadline - now : 0;
  const level = isLive ? responseLevel(remaining) : "idle";
  const ringLevel = trRingLevel(level);
  const totalWindow =
    request.deadline !== null && request.requestedAt !== null && request.deadline > request.requestedAt
      ? request.deadline - request.requestedAt
      : 60 * 60000;
  const ratio = isLive ? Math.min(1, remaining / totalWindow) : 0;

  let closedLabel = "Requested";
  let closedValue = requestedIso ? clockTime(requestedIso) : "--:--";
  if (request.status === "accepted") closedLabel = "Accepted · req";
  else if (request.status === "rejected") closedLabel = "Rejected · req";
  else if (request.status === "new" && request.deadline === null) {
    closedLabel = "Window";
    closedValue = "Not set";
  } else if (request.status === "new") {
    closedLabel = "Window closed";
    closedValue = clockTime(new Date(request.deadline).toISOString());
  }

  const open = () => onView(request);
  const handleKeyDown = event => {
    if (event.key === "Enter" || event.key === " ") {
      event.preventDefault();
      open();
  ...
  return (
    <article
      className={
        "rq-card rq-tr-compact status-" + cls + " urg-" + (isLive ? ringLevel : "idle")
      }
      ...
      onClick={open}
      onKeyDown={handleKeyDown}
      aria-label={"Open transporter request " + (request.id || "without ID")}
    >
      <span className="rq-card-rail" aria-hidden="true" />
      <span className="rq-card-sheen" aria-hidden="true" />
      <span className="rq-card-mesh" aria-hidden="true" />
      ...
      <header className="rq-card-head">
        <span className="rq-id">
          <span className="rq-id-dot" />
          <span className={"rq-id-text" + (request.id ? "" : " rq-missing")} title={request.id || undefined}>
            {request.id || "Request ID unavailable"}
          </span>
        </span>
        <span className={"rq-badge " + cls}>
          <i className="rq-badge-pip" />
          {request.statusLabel}
        </span>
      </header>
      ...
      <div className="rq-buyer">
        ...
        <span className={request.transporterName ? "" : "rq-missing"}>
          {request.transporterName || "Transporter name not provided"}
          {request.transporterId ? " · " + request.transporterId : ""}
          {request.ownerName ? " · " + request.ownerName : ""}
        </span>
      </div>

      <div className="rq-materials" aria-label="Material and seller">
        <span className="rq-chip">
          <i className="rq-chip-icon"><Icon name="cube" size={12} /></i>
          <span className={"rq-chip-name" + (request.materialName ? "" : " rq-missing")}>
            {request.materialName || "Material N/A"}
          </span>
          {hasQuantity ? (
            <b>
        ...
        <span className="rq-chip total">
          <i className="rq-chip-icon"><Icon name="user" size={12} /></i>
          <span className={"rq-chip-name" + (request.sellerName || request.sellerId ? "" : " rq-missing")}>
            {request.sellerName || (request.sellerId ? "Seller" : "Seller not assigned")}
          </span>
          {request.sellerId ? <b>{request.sellerId}</b> : null}
        </span>
      </div>

      <div className="rq-meta">
        <div className="rq-meta-cell" title={request.pickupLocation || undefined}>
          <Icon name="pin" size={13} />
          <span>Pickup</span>
          <strong className={request.pickupLocation ? "" : "rq-missing"}>
            {request.pickupLocation || "N/A"}
          </strong>
        </div>
        <div className="rq-meta-cell" title={requestedFull || undefined}>
          <Icon name="clock" size={13} />
          <span>Requested</span>
          <strong className={requestedIso ? "" : "rq-missing"}>
            {requestedIso ? relativeTime(requestedIso, now) : "N/A"}
          </strong>
        </div>
      </div>
      ...
      <footer className="rq-card-foot">
        {isLive ? (
          <CountdownRing
            ...
            value={trCountdown(remaining)}
          />
        ) : (
          <div className={"rq-closed " + cls}>
            <Icon name="flag" size={13} />
            <span>{closedLabel}</span>
            <strong>{closedValue}</strong>
          </div>
        )}
        <span className="rq-open">
          View request
...
function TransporterRequestsView({
  ...
  if (error) {
    ...
    content = (
      <section className="rq-list" role="status" aria-live="polite" aria-label="Loading Transporter requests">
    ...
    content = (
      <section className="rq-list" aria-label="Transporter requests">
...
export default function AdminRateRequests({
  ...
  const transporterStats = useMemo(() => {
    ...
    let accepted = 0;
    let rejected = 0;
    transporterItems.forEach(item => {
      if (item.status === "accepted") accepted += 1;
      else if (item.status === "rejected") rejected += 1;
      else if (item.status === "new") {
        const open = item.deadline === null || item.deadline - now > 0;
        if (open) live += 1;
        if (item.deadline !== null && responseLevel(item.deadline - now) === "critical") critical += 1;
      }
    });
    const decided = accepted + rejected;
    return {
      total: transporterItems.length,
      live,
      critical,
      acceptRate: decided ? Math.round((accepted / decided) * 100) : 0
    };
  }, [transporterItems, now]);
        ...
        <div className="rq-header-inner">
          ...
          <div className="rq-metrics">
            ...
            <Metric
              ...
              tone="red"
              label="Critical"
              value={transporterStats.critical}
              unit=" urgent"
              hint="Closing in <15m"
              bars={[20, 34, 26, 58, 40, 74]}
            />
            <Metric
              ...
              tone="slate"
              label="Requests"
              value={transporterStats.total}
              unit=" total"
              hint="From transporters"
              bars={[46, 30, 68, 52, 84, 60]}
            />
            <Metric
              icon="target"
              tone="green"
              label="Accept rate"
              value={transporterStats.acceptRate}
              unit="%"
              hint="Accepted vs decided"
              bars={[30, 48, 40, 66, 58, 88]}
            />
...
/* compact transporter card (buyer-card look) */
.rq-card.status-new .rq-badge,.rq-card .rq-badge{white-space:nowrap}
.rq-tr-compact .rq-id{flex:1}
.rq-tr-compact .rq-chip{max-width:100%}
.rq-tr-compact .rq-closed span{white-space:nowrap}
.rq-tr-compact .rq-ring-text strong{letter-spacing:0}
